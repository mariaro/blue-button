...
Cleanup.clearNulls = function () {
    ...
    if ('object' === typeof this.js) {
        Object.keys(this.js).forEach(function (k) {
            if ((this.js[k] === null) || (this.js[k] === undefined) ||
...
Cleanup.cleanupConceptDescriptor = function () {
    if(!this.js)
        return;

    if (common.exists(this.js.nullFlavor) && !common.exists(this.js.originalText) && !common.exists(this.js.translations)) {
        this.js = null;
        return;
    }
};
...
Cleanup.flattenCollection = function (field) {
    var f = function () {
        if (this.js && this.js[field] && Array.isArray(this.js[field]) && this.js[field].length > 0) {
            var list = [];
            _.each(this.js[field], function(mi){
                if (_.isArray(mi.js)) {
                    _.each(mi.js, function(mii){
                        list = list.concat(mii.js);
                    });
                }
                else {
                    list = list.concat(mi.js);
                }
            });
            this.js[field] = list;
        }
