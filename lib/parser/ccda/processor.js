...
var asTimestamp = Processor.asTimestamp = function (v) {
    ...
    //Fixing ignored timezone:

    var isoString;
    //if it has time component
    if(t.length > 8) {
        //but no timezone => use server timezone
        if(!/[\+\-].+/.test(t)) {
            isoString = moment(t, 'YYYYMMDDHHmmss').toISOString();
        }
        //has timezone
        else {
            isoString = moment(t, 'YYYYMMDDHHmmssZ').toISOString();
        }
    ...
    if(t.length === 0) //empty string
        return ''; //new XDate(0, 0, 1, 0, 0, 0, 0).toISOString();

    //fail, return undefined
    // END fixing ignored timezone
